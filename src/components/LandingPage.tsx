...
export default function LandingPage() {
    ...
    // Data for steps to keep JSX clean and avoid inline styles
    const steps = [
        {
            step: "01",
            title: "Authenticate",
            desc: "Login via Mobile OTP",
            numberColor: "text-[var(--wb-gold)]",
            bgColor: "bg-yellow-50",
            borderColor: "border-[var(--wb-gold)]",
            accentColor: "bg-[var(--wb-gold)]"
        },
        {
            step: "02",
            title: "Details",
            desc: "Enter Vehicle Number",
            numberColor: "text-[var(--wb-emerald)]",
            bgColor: "bg-emerald-50",
            borderColor: "border-[var(--wb-emerald)]",
            accentColor: "bg-[var(--wb-emerald)]"
        },
        {
            step: "03",
            title: "e-Sign",
            desc: "Aadhaar Verification",
            numberColor: "text-[var(--wb-indigo)]",
            bgColor: "bg-indigo-50",
            borderColor: "border-[var(--wb-indigo)]",
            accentColor: "bg-[var(--wb-indigo)]"
        },
        {
            step: "04",
            title: "Approval",
            desc: "Digital RC Issued",
            numberColor: "text-[var(--wb-green)]",
            bgColor: "bg-green-50",
            borderColor: "border-[var(--wb-green)]",
            accentColor: "bg-[var(--wb-green)]"
        },
    ]
    ...
    return (
        <div className="min-h-screen bg-[#fafaf9] text-stone-800 font-body relative selection:bg-[var(--wb-gold)] selection:text-[var(--wb-dark)]">
            ...
            {/* Main Content Wrapper - overflow-x-hidden moved here */}
            <div className="overflow-x-hidden">
                {/* Hero Section */}
                <section className="relative pb-6 pt-[-10px] md:px-10 overflow-hidden z-10">

                    {/* --- MOVING GEOMETRIC BACKGROUNDS --- */}
                    {/* 1. Top Right: Large Emerald Circle (Breathing) */}
                    <div className="absolute top-0 right-0 w-[45vw] h-[45vw] bg-[var(--wb-emerald)] rounded-full -translate-y-1/4 translate-x-1/4 border border-[var(--wb-emerald)]/20 -z-10 animate-pulse-gentle"></div>

                    {/* 2. Top Left: Gold/Amber Circle (Floating Up/Down) */}
                    <div className="absolute top-10 left-10 w-64 h-64 bg-[var(--wb-gold)]/5 rounded-full -translate-x-1/2 -translate-y-1/4 border border-[var(--wb-gold)]/20 -z-10 animate-float-slow"></div>

                    {/* 3. Bottom Center/Left: Primary Blue Circle (Drifting Sideways) */}
                    <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-[var(--wb-primary)]/5 rounded-full translate-y-1/4 border border-[var(--wb-primary)]/10 -z-10 animate-float-medium"></div>
                    {/* ------------------------------------ */}

                    <div className="max-w-7xl mx-auto pt-0 pb-6 grid lg:grid-cols-2 gap-16 items-center">
                        ...
                        <div className="relative lg:h-[600px] flex items-center justify-center mt-12 lg:mt-0">
                            {/* Decorative solid shape behind image */}
                ...
                <section className="py-24 max-w-7xl mx-auto px-10 relative z-10">
                    ...
                    <div className="grid md:grid-cols-4 gap-6">
                        {steps.map((item, i) => (
                            <div key={i} className={`relative group p-6 rounded-2xl border-2 transition-all duration-300 hover:-translate-y-2 ${item.bgColor} border-transparent`}>
                                {/* Top accent line */}
                                <div className={`absolute top-0 left-0 w-full h-1.5 rounded-t-2xl opacity-60 ${item.accentColor}`}></div>

                                <div className="pt-4">
                                    <span className={`text-5xl font-heading font-bold block mb-4 opacity-40 ${item.numberColor}`}>
                                        {item.step}
                                    </span>
